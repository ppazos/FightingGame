...
SFont_Font* SFont_InitFont(SDL_Surface* Surface)
{
    ...
    Uint32 cyan = SDL_MapRGB(Surface->format, 255, 0, 255);
    while (x < Surface->w) {
	if (GetPixel(Surface, x, 0) == cyan) { 
    	    Font->CharPos[i++]=x;
    	    while((x < Surface->w) && (GetPixel(Surface, x, 0)==cyan))
		x++;
	    Font->CharPos[i++]=x;
	}
	x++;
...
*/
    dstrect.x = int(x - (float)(Font->CharPos[charoffset] - Font->CharPos[charoffset-1])/2);
